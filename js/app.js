...
function getlocation() {
   ...
   navigator.geolocation.getCurrentPosition(onceFoundLocation);   //Finds the position then calls a function
}

function onceFoundLocation(position) {   //callback function that runs once location is found
         ...
         if ($.inArray('locality', location.types) != -1) {
           ...
           $.getJSON(jsonURL, function (json) {
            ...
            if(rain){
                ...
                document.getElementById("rainDisp").innerHTML = "There's been " + rainFall + "mm of rainfall today so don't worry about a raincoat.";
            }
           });
           break;
         }
...
function aFunct(number, callback){
    var bit = number +1;   //callback understanding experiment
    callback(bit);
};
